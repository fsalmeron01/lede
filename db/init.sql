...
CREATE TABLE IF NOT EXISTS jobs (
  ...
  title TEXT,
  content_mode TEXT NOT NULL DEFAULT 'camden-tribune',
...
CREATE TABLE IF NOT EXISTS publish_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  job_id UUID NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
  target_type TEXT NOT NULL DEFAULT 'wordpress',
...
CREATE INDEX IF NOT EXISTS idx_publish_log_job_id ON publish_log(job_id);
...
CREATE TRIGGER trg_summaries_updated_at BEFORE UPDATE ON summaries FOR EACH ROW EXECUTE FUNCTION set_updated_at();
